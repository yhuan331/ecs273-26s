...
export function LineChart({ ticker }) {
  ...
  return (
    <div ref={containerRef} style={{ width: "100%", height: "100%", overflowX: "auto", position: "relative" }}>
...
function drawChart(svgElement, data, width, height, ticker) {
  ...
  const totalWidth = Math.max(width, data.length * 3);
  ...
  g.append("g").attr("class", "x-axis")
    ...
    .call(d3.axisBottom(xScale).ticks(Math.min(12, data.length / 20)))
  ...
  // ── Title ─────────────────────────────────────────────────────────────────
  svg.append("text")
    .attr("x", totalWidth / 2).attr("y", margin.top - 4)
  ...
  g.append("rect")
    ...
    .on("mousemove", function (event) {
      ...
      // Tooltip position — offset from cursor, flip if near right edge
      const parentRect = svgElement.parentNode.getBoundingClientRect();
      const relX = event.clientX - parentRect.left + svgElement.parentNode.scrollLeft;
      ...
      tooltip.style("display", "block")
        ...
        .html(`
          <div style="font-weight:700;margin-bottom:4px;border-bottom:1px solid #334155;padding-bottom:4px;color:#94a3b8">
            ${dateFmt(d.date)}
          </div>
  ...
  const zoom = d3.zoom()
    ...
    .on("zoom", (event) => {
      ...
      g.select(".x-axis").call(d3.axisBottom(currentXScale).ticks(10));
      lines.forEach(({ key }) => {
        g.select(`#line-${key}`)
          .attr("d", d3.line()
